...
- **Исправлены 4 критические ошибки в Slave-прошивке (`ESP32_S3_ATTACK.ino`)**:
  ...
  * **Аппаратная инициализация**: В функции `setup()` добавлены подтяжка `IR_RX_PIN` (`INPUT_PULLUP`) и перевод пина `CC1101_CS` в режим `OUTPUT` с уровнем `HIGH` для предотвращения конфликтов устройств на SPI-шине.
...
- **Предотвращение срабатывания WDT (Пункт 8)**: добавлены вызовы `yield()` во все циклы атак и сканирования для сброса аппаратного сторожевого таймера.
